...
import { 
  ...
  Volume2, VolumeX, KeyRound
} from 'lucide-react';

export default function App() {
  // Google Apps Script URL for Global Time Locking
  const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbw_WcIeCGUDpropWAKiEwiudUkqMZb84G0ZVTzRORN89FuqOhjK7vO-Wk4DC-f7AxAc/exec";
  ...
  // 1. Fetch permanently saved time globally from Google Sheets
  useEffect(() => {
    fetch(GOOGLE_SCRIPT_URL)
      .then(res => res.json())
      .then(data => {
        if (data && data.time) {
          setYesTime(data.time);
        }
        setIsFetchingTime(false);
      })
      .catch(err => {
        console.error("Error fetching time from Google Sheets:", err);
        setIsFetchingTime(false); 
      });
  }, []);
  ...
  useEffect(() => {
    if (currentView === 'loading' && !isFetchingTime) {
      const timer = setTimeout(() => {
        navigateTo('home');
      }, 5500); 
      return () => clearTimeout(timer);
    }
  }, [currentView, isFetchingTime]);
  ...
  const lockTimeForever = (time) => {
    ...
    fetch(GOOGLE_SCRIPT_URL, {
      ...
      body: new URLSearchParams({ action: 'save', time: time })
  ...
  const unlockTime = () => {
    if (adminPassword === 'sakshamlovesmeghna') {
      ...
      .then(() => {
        alert("⚠️ Time removed successfully! The site will now refresh to start over.");
        window.location.reload(); 
      })
      .catch(err => alert("Error resetting in Google Sheets"));
    } else {
  ...
  if (displayView === 'loading' || isFetchingTime) {
    return <LoadingScreen onComplete={() => navigateTo('home')} />;
  }

  const showFloatingEffects = ['home', 'family', 'gallery', 'location', 'journey'].includes(displayView);
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-pink-50 via-rose-50 to-pink-100 text-rose-900 font-sans selection:bg-pink-300 selection:text-white flex flex-col items-center pt-10 pb-4 px-2 sm:px-4 overflow-hidden">
      ...
      <audio ref={audioRef} src="bg-music.mp3" loop className="hidden" />
      ...
      <div className={`relative z-10 w-full flex flex-col items-center justify-center min-h-[90vh] transition-opacity duration-500 ease-in-out ${isFading ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
        ...
        {displayView === 'gallery' && <GalleryView onNext={() => navigateTo('location')} onBack={() => navigateTo('family')} />}
        {displayView === 'location' && <LocationView onNext={() => navigateTo('journey')} onBack={() => navigateTo('gallery')} />}
        {displayView === 'journey' && <JourneyView onNext={() => navigateTo('proposal')} onBack={() => navigateTo('location')} />}
        ...
        {displayView === 'certificate' && <CertificateView time={yesTime} onNext={() => navigateTo('final')} onOpenReset={() => setShowAdminModal(true)} />}
      ...
      {showAdminModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-sm w-full animate-fade-in border-4 border-rose-200">
            <h3 className="text-2xl font-bold text-rose-800 mb-2 flex items-center gap-2">
              <Lock className="w-6 h-6" /> Reset Agreement
            </h3>
            <p className="text-sm text-rose-600 mb-6 font-medium">⚠️ Warning: This will remove the locked time and restart the entire experience from scratch!</p>
            <input 
              type="password" 
              value={adminPassword}
            ...
            <div className="flex gap-3 justify-end">
              <button 
                ...
                className="cursor-pointer px-5 py-3 font-bold text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200 transition-all w-full"
              >
                Cancel
              </button>
              <button 
                onClick={unlockTime}
                className="cursor-pointer px-6 py-3 font-bold text-white bg-rose-600 rounded-xl hover:bg-rose-700 shadow-md transition-all w-full"
              >
                Reset Site
              </button>
            </div>
  ...
  return (
    <div className={`fixed inset-0 bg-gradient-to-br from-pink-50 via-rose-100 to-pink-100 animate-gradient-xy flex flex-col items-center justify-center z-50 transition-opacity duration-500 ${isFadingOut ? 'opacity-0' : 'opacity-100'}`}>
      ...
      <div className="absolute bottom-24 w-64 md:w-96 h-2 bg-rose-200/50 rounded-full overflow-hidden shadow-inner z-10">
        <div className="h-full bg-rose-500 rounded-full transition-all duration-75 ease-linear shadow-[0_0_12px_rgba(244,63,94,0.6)] relative" style={{ width: `${progress}%` }}>
...
function BackgroundEffects() {
  const elements = Array.from({ length: 15 }).map((_, i) => ({
    id: i, left: `${Math.random() * 100}%`, animationDuration: `${10 + Math.random() * 15}s`, animationDelay: `${Math.random() * 5}s`, isDog: i % 3 === 0 
  }));
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-0 opacity-40">
      {elements.map((el) => (
        <div key={el.id} className="absolute bottom-0 text-pink-300 flex items-center justify-center" style={{ left: el.left, animation: `float-up ${el.animationDuration} linear infinite`, animationDelay: el.animationDelay, fontSize: `${1 + Math.random() * 1.5}rem` }}>
          {el.isDog ? '🐾' : '💖'}
        </div>
      ))}
    </div>
...
function PetalEffects() {
  ...
  const confettiPieces = Array.from({ length: 45 }).map((_, i) => ({
    id: i, left: `${Math.random() * 100}%`, animationDuration: `${4 + Math.random() * 6}s`, animationDelay: `${Math.random() * 3}s`, color: petalColors[Math.floor(Math.random() * petalColors.length)], size: `${10 + Math.random() * 15}px`,
  }));
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-20">
      {confettiPieces.map((c) => (
        <div key={c.id} className="absolute top-0 opacity-90 shadow-sm" style={{ left: c.left, width: c.size, height: c.size, backgroundColor: c.color, borderRadius: '50% 0 50% 0', animation: `petal-fall ${c.animationDuration} linear infinite`, animationDelay: c.animationDelay }} />
      ))}
    </div>
  );
}
    ...
    <div className="animate-fade-in w-full max-w-6xl mx-auto py-10 px-2 sm:px-4">
      <div className="text-center mb-12 mt-8">
        <h2 className="text-4xl md:text-5xl font-serif font-bold text-rose-800 mb-4 flex justify-center items-center gap-3 px-2">
          <Dog className="w-8 h-8 md:w-10 md:h-10 text-rose-500 flex-shrink-0" /> Our Fur Babies <Dog className="w-8 h-8 md:w-10 md:h-10 text-rose-500 flex-shrink-0" />
        </h2>
        ...
        {familyMembers.map((dog, idx) => (
          <div key={idx} className={`bg-gradient-to-br ${dog.color} p-6 rounded-3xl shadow-xl border-4 border-white transform hover:-translate-y-2 transition-all duration-300 flex flex-col items-center text-center group`}>
            <div className="w-28 h-28 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-white shadow-inner mb-4 bg-white/50 flex items-center justify-center relative">
              <img src={dog.image} alt={dog.type} className="w-full h-full object-cover absolute inset-0 z-10" onError={(e) => { e.target.style.display = 'none'; }} />
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-4 px-4 w-full max-w-md mx-auto sm:max-w-none">
        <button onClick={onBack} className="cursor-pointer w-full sm:w-auto justify-center px-8 py-4 font-bold text-rose-700 bg-white/80 rounded-full shadow-md hover:bg-white transition-colors border-2 border-rose-200 flex items-center gap-2"><ArrowLeft className="w-5 h-5" /> Back</button>
        <button onClick={onNext} className="cursor-pointer w-full sm:w-auto justify-center px-8 py-4 font-bold text-white bg-rose-500 rounded-full shadow-lg hover:bg-rose-600 transition-all flex items-center gap-2">Our Memories <ArrowRight className="w-5 h-5" /></button>
      </div>
    </div>
  );
}
    ...
    <div className="animate-fade-in w-full max-w-6xl mx-auto py-10 px-4">
      <div className="text-center mb-8 mt-8">
        <h2 className="text-4xl md:text-5xl font-serif font-bold text-rose-800 mb-4 flex justify-center items-center gap-3">
          <Camera className="w-8 h-8 md:w-10 md:h-10 text-rose-500" /> Our Gallery <ImageIcon className="w-8 h-8 md:w-10 md:h-10 text-rose-500" />
        </h2>
      ...
      <div className="masonry-grid mb-12 px-2 md:px-6">
        {potentialPhotos.map((src, idx) => (
          <img key={`local-${idx}`} src={src} alt={`Memory ${idx + 1}`} className="masonry-item w-full rounded-2xl shadow-lg border-4 border-white hover:scale-[1.02] transition-transform duration-300" onError={(e) => { e.target.style.display = 'none'; }} />
        ))}
      </div>
      <div className="flex flex-col sm:flex-row justify-center gap-4 px-4 w-full max-w-md mx-auto sm:max-w-none">
        <button onClick={onBack} className="cursor-pointer w-full sm:w-auto justify-center px-8 py-4 font-bold text-rose-700 bg-white/80 rounded-full shadow-md hover:bg-white transition-colors border-2 border-rose-200 flex items-center gap-2"><ArrowLeft className="w-5 h-5" /> Fur Babies</button>
        <button onClick={onNext} className="cursor-pointer w-full sm:w-auto justify-center px-8 py-4 font-bold text-white bg-gradient-to-r from-rose-500 to-pink-500 rounded-full shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center gap-2">Where We Met <ArrowRight className="w-5 h-5" /></button>
      </div>
    </div>
  );
}

function LocationView({ onNext, onBack }) {
  const lat = "12.9962244"; 
  const lng = "77.5824505"; 

  return (
    <div className="animate-fade-in w-full max-w-5xl mx-auto py-10 px-4">
      <div className="text-center mb-10 mt-8">
        <h2 className="text-4xl md:text-5xl font-serif font-bold text-rose-800 mb-4 flex justify-center items-center gap-3">
          <MapPin className="w-8 h-8 md:w-10 md:h-10 text-rose-500" /> 
          The Place We First Met 
          <MapPin className="w-8 h-8 md:w-10 md:h-10 text-rose-500" />
        </h2>
        <p className="text-lg md:text-xl text-rose-600 mb-6 italic">The exact spot where I met the best person in this entire universe.</p>
      </div>

      <div className="bg-white/80 backdrop-blur-md p-4 md:p-6 rounded-3xl shadow-2xl border-4 border-rose-200 mb-12 w-full mx-auto">
         <iframe
            title="Where we met"
...
function JourneyView({ onNext, onBack }) {
  const journeySteps = [
    { icon: <Dog className="w-6 h-6 md:w-8 md:h-8 text-rose-500" />, title: "Rescuing Love", desc: "It all started with saving some little paws. Rescuing those puppies didn't just save them, it brought our worlds together. Seeing your heart for them made me fall for you." },
    { icon: <Coffee className="w-6 h-6 md:w-8 md:h-8 text-rose-500" />, title: "Coffee & Conversations", desc: "Then came the long talks and exploring every new coffee place we could find. With every sip of coffee and every shared laugh, I knew you were the one." },
    { icon: <Moon className="w-6 h-6 md:w-8 md:h-8 text-rose-500" />, title: "Late Night Cravings", desc: "Taking you to late night coffee, maggi, and idli dosa dates... I loved every single moment I spent with you. Simple moments became my absolute favorite memories." },
    { icon: <Mountain className="w-6 h-6 md:w-8 md:h-8 text-rose-500" />, title: "Kedarnath Diaries", desc: "Our spiritual journey to Kedarnath. Under the blessings of Mahadev (ॐ), the freezing cold breeze, the majestic peaks... everything was perfect because I held your hand." },
    { icon: <Waves className="w-6 h-6 md:w-8 md:h-8 text-rose-500" />, title: "The Holy Ganga Dupki", desc: "Taking that holy dip together in the Ganga. It felt like a cleansing of our souls and an unspoken promise that no matter what, we face life's currents together." }
  ];
  return (
    <div className="animate-fade-in w-full max-w-4xl mx-auto py-10 px-2 sm:px-6">
      ...
      <div className="space-y-12 relative before:absolute before:inset-0 before:ml-[1.75rem] md:before:mx-auto md:before:translate-x-0 before:h-full before:w-1.5 before:bg-gradient-to-b before:from-rose-300 before:via-pink-500 before:to-rose-300 before:rounded-full px-2">
        {journeySteps.map((step, index) => (
          <div key={index} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
            <div className="flex items-center justify-center w-12 h-12 md:w-16 md:h-16 rounded-full border-4 border-white bg-pink-100 text-rose-500 shadow-xl shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 relative z-10 transform group-hover:scale-110 transition-transform">{step.icon}</div>
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-4 mt-20 px-4 w-full max-w-md mx-auto sm:max-w-none">
        <button onClick={onBack} className="cursor-pointer w-full sm:w-auto justify-center px-8 py-4 font-bold text-rose-700 bg-white/80 rounded-full shadow-md hover:bg-white transition-colors border-2 border-rose-200 flex items-center gap-2"><ArrowLeft className="w-5 h-5" /> Map</button>
        <button onClick={onNext} className="cursor-pointer w-full sm:w-auto justify-center inline-flex items-center gap-2 px-10 py-4 font-bold text-white bg-gradient-to-r from-rose-600 to-red-500 rounded-full shadow-2xl hover:shadow-rose-500/50 hover:scale-105 transition-all duration-300">The Big Question <Heart className="w-5 h-5 fill-white animate-pulse" /></button>
      </div>
    </div>
  );
}
  ...
  if (isLocked) {
    return (
      <div className="animate-fade-in flex flex-col items-center justify-center min-h-[60vh] text-center px-6 py-10 w-full max-w-xl mx-auto">
        ...
        <button onClick={onViewCertificate} className="cursor-pointer w-full sm:w-auto px-10 py-5 bg-gradient-to-r from-rose-500 to-pink-500 text-white font-bold text-xl rounded-full shadow-xl hover:scale-105 transition-all">View Our Certificate</button>
      </div>
    );
  }

  const getNoButtonText = () => {
    const phrases = [ "No", "Are you absolutely sure?", "Please say yes Meghna!", "Think about the dogs!", "They need their parents together!", "Don't do this to Saksham!", "You're breaking my heart ;(", "I'm gonna cry...", "It won't work anyway, say yes!" ];
    return phrases[Math.min(noCount, phrases.length - 1)];
  };

  const handleNoClick = () => {
    setNoCount(prev => prev + 1);
    setNoPosition({ x: (Math.random() - 0.5) * (window.innerWidth < 600 ? 150 : 250), y: (Math.random() - 0.5) * (window.innerWidth < 600 ? 150 : 250) });
  };

  const handleYesClick = () => {
    const now = new Date();
    const timeString = now.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' });
    onYes(timeString);
  };
  ...
  return (
    <div className="animate-fade-in flex flex-col items-center justify-center min-h-[85vh] w-full text-center px-4 mt-8 overflow-hidden">
      <div className="mb-10 relative">
        <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExcDdtZ2JiZDRqZ3JzN29jcnV6eHpwZXg5MnUyeHhjY2Y4cWF0d24yMyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/MDJ9IbxxvDUQM/giphy.gif" alt="Cute pleading cat" className="w-48 h-48 sm:w-56 sm:h-56 md:w-64 md:h-64 object-cover rounded-full mx-auto mb-8 shadow-2xl border-8 border-white z-10 relative" />
        <h1 className="text-4xl sm:text-5xl md:text-7xl font-serif font-bold text-rose-800 leading-tight drop-shadow-sm px-2">Meghna, will you be <br/> my wife forever?</h1>
      </div>
      <div className="flex flex-col md:flex-row items-center justify-center gap-6 md:gap-10 w-full relative z-20 min-h-[250px] px-4">
        <button onClick={handleYesClick} style={{ transform: `scale(${yesButtonSize})`, zIndex: 50 }} className="cursor-pointer px-12 py-5 bg-gradient-to-r from-rose-600 via-pink-600 to-red-600 text-white text-2xl font-bold rounded-full shadow-2xl transition-all duration-300 hover:shadow-rose-500/50 hover:bg-rose-700 border-2 border-white/20 w-full md:w-auto">YES! 💍💖</button>
        <button onClick={handleNoClick} onMouseEnter={handleNoClick} style={{ transform: `translate(${noPosition.x}px, ${noPosition.y}px)`, position: noCount > 0 ? 'absolute' : 'relative', zIndex: 40 }} className="cursor-pointer px-8 py-4 bg-white text-rose-500 text-lg font-semibold rounded-full shadow-md border-2 border-rose-200 transition-all duration-200 hover:bg-gray-50 whitespace-nowrap">{getNoButtonText()}</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="animate-fade-in flex flex-col items-center justify-center min-h-[80vh] text-center px-4 max-w-2xl mx-auto w-full">
      <div className="bg-white p-8 md:p-12 rounded-3xl shadow-2xl border-4 border-pink-200 relative overflow-hidden w-full">
        ...
        <p className="text-sm md:text-base text-gray-500 mb-10 italic px-2">Locking this time will stamp it permanently onto your certificate. Even if you refresh the website or change devices, it will remember this exact moment forever.</p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button onClick={onCancel} className="cursor-pointer w-full sm:w-auto px-8 py-4 font-bold text-rose-600 bg-white rounded-full border-2 border-rose-200 hover:bg-rose-50 transition-all text-lg">Wait, go back</button>
          <button onClick={onConfirm} className="cursor-pointer w-full sm:w-auto px-8 py-4 font-bold text-white bg-gradient-to-r from-rose-500 to-rose-600 rounded-full shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all flex items-center justify-center gap-2 text-lg"><Lock className="w-5 h-5" /> Stamp it Forever!</button>
        </div>
      </div>
    </div>
  );
...
function CertificateView({ time, onNext, onOpenReset }) {
  ...
  const printCertificate = async () => {
    ...
    try {
      const canvas = await html2canvas(element, { scale: 2, backgroundColor: "#fffaf5", useCORS: true, logging: false });
    ...
    <div className="animate-fade-in flex flex-col items-center w-full max-w-[100vw] px-0 md:px-4 py-10 mt-4 md:mt-10 overflow-hidden">
      ...
      <div className="no-print text-center mb-8 px-4 w-full">
        ...
        <h2 className="text-3xl md:text-4xl font-bold text-rose-700 flex items-center justify-center gap-3 mb-4 flex-wrap">
          <Sparkles className="w-8 h-8 text-yellow-500 flex-shrink-0" /> MEGHNA SAID YES! <Sparkles className="w-8 h-8 text-yellow-500 flex-shrink-0" />
        </h2>
      ...
      <div id="certificate-wrapper" className="w-full overflow-x-auto custom-scrollbar pb-6 px-4 md:px-0">
        <div id="certificate-area" className="min-w-[800px] max-w-5xl w-full bg-[#fffaf5] p-12 md:p-16 border-[16px] border-double border-rose-200 shadow-xl relative rounded-sm mx-auto">
          ...
          <div className="absolute top-10 left-0 w-full flex justify-center opacity-[0.15] pointer-events-none"><div className="text-[10rem] font-serif text-rose-400">ॐ</div></div>
          
          <div className="text-center space-y-12 relative z-10 pt-4">
            <div className="inline-block">
              <h1 className="text-5xl md:text-6xl font-serif text-rose-900 tracking-widest uppercase border-b-4 border-rose-300 pb-5">Agreement of Forever</h1>
            ...
            <div className="py-10 space-y-8">
              ...
              <h2 className="text-5xl font-bold text-rose-700 font-serif border-b-2 border-dashed border-rose-300 inline-block pb-3 px-12">Meghna Kantak</h2>
              <p className="text-xl md:text-2xl text-gray-700 italic font-serif leading-relaxed px-12 max-w-3xl mx-auto">
                has agreed to tolerate, love, and annoy <br/><span className="font-bold text-rose-800 text-3xl md:text-4xl mt-3 mb-1 inline-block">Saksham Talwar</span> <br/>and co-parent all our 5 furry babies for the rest of her natural life.
              </p>
            </div>
            <div className="bg-pink-50 p-8 rounded-2xl max-w-md mx-auto border-2 border-rose-100 shadow-inner">
              <p className="text-rose-900 font-bold uppercase tracking-widest text-sm flex justify-center items-center gap-2"><Lock className="w-4 h-4" /> Officially Sealed On:</p>
            ...
            <div className="pt-16 flex justify-between items-end px-12 pb-4">
              <div className="text-center w-48">
                <div className="border-b-2 border-rose-500 mb-3 pb-3 h-20 flex items-end justify-center"><span className="font-serif text-4xl text-rose-900 italic">Saksham</span></div>
              ...
              <div className="w-32 h-32 bg-gradient-to-br from-rose-500 to-red-600 rounded-full flex items-center justify-center border-4 border-rose-200 shadow-2xl transform rotate-12 relative mx-4">
                 <div className="absolute inset-2 border-2 border-dashed border-white/70 rounded-full"></div><span className="text-white font-serif text-4xl font-bold italic drop-shadow-md">S & M</span>
              </div>
              <div className="text-center w-48">
                <div className="border-b-2 border-rose-500 mb-3 pb-3 h-20 flex items-end justify-center"><span className="font-serif text-4xl text-rose-900 italic">Meghna</span></div>
      ...
      <div className="no-print mt-10 flex flex-col sm:flex-row gap-4 md:gap-6 w-full max-w-2xl px-4">
        <button onClick={printCertificate} disabled={isSaving} className="cursor-pointer flex-1 flex items-center justify-center gap-2 px-8 py-5 bg-white text-rose-700 font-bold text-lg rounded-full shadow-lg hover:bg-rose-50 hover:-translate-y-1 transition-all border-2 border-rose-200 disabled:opacity-50 disabled:hover:translate-y-0">
          {isSaving ? <span className="animate-pulse">Saving...</span> : <><Download className="w-6 h-6" /> Save Image</>}
        </button>
        <button onClick={onNext} className="cursor-pointer flex-1 flex items-center justify-center gap-2 px-8 py-5 bg-gradient-to-r from-rose-600 to-pink-600 text-white font-bold text-lg rounded-full shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all">
          One Last Surprise <ArrowRight className="w-6 h-6" />
        </button>
      </div>

      {/* Visible Reset Key Icon at the bottom of the certificate screen */}
      <div className="no-print mt-8">
        <button 
          onClick={onOpenReset}
          className="cursor-pointer flex items-center gap-2 text-rose-400 hover:text-rose-600 text-xs font-semibold tracking-wider uppercase transition-colors py-2 px-4 rounded-full border border-dashed border-rose-200 hover:border-rose-400 bg-white/50"
        >
          <KeyRound className="w-4 h-4" /> Reset Agreement Time
        </button>
      </div>
    </div>
  );
...
function FinalView() {
  ...
  const handleInputChange = (e) => {
    ...
    // Flexible checking: accepts "ily", "i love you", "1000. i love you", or similar variations
    if (
      val === 'ily' || 
      val.includes('i love you') || 
      val.includes('1000') ||
    ...
    return (
      <div className="animate-fade-in flex flex-col items-center justify-center text-center min-h-[90vh] w-full px-4 relative z-20 mt-10 py-10">
        ...
        <div className="relative mb-12">
          ...
          <div className="absolute top-0 left-0 w-full h-full animate-ping opacity-60"><Heart className="w-40 h-40 md:w-56 md:h-56 text-pink-400 fill-pink-400 mx-auto" /></div>
        </div>
        <h1 className="text-5xl sm:text-7xl md:text-8xl font-serif font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-600 via-rose-600 to-pink-600 mb-8 drop-shadow-lg px-2">WE ARE FOREVER!</h1>
        <div className="border-t-2 border-b-2 border-rose-200 py-10 px-4 w-full max-w-4xl bg-white/40 backdrop-blur-sm rounded-3xl z-10">
          <p className="text-2xl md:text-4xl text-rose-900 leading-relaxed font-serif italic">1000 times I love you, and a million more to go. Thank you for making me the happiest man, and making our 5 furry babies a complete family. <br/><br/> Our forever begins right now. 💖🐾</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade-in w-full max-w-4xl flex flex-col items-center py-10 px-4 mt-10">
      <h2 className="text-4xl md:text-6xl font-serif font-bold text-rose-900 text-center mb-6">Just One Last Thing, Meghna...</h2>
      <p className="text-lg sm:text-xl md:text-2xl text-rose-700 text-center max-w-3xl mb-12 leading-relaxed font-medium px-4">I wanted to prove how much I love you. So I started counting...</p>
      <div className="w-full bg-white/90 backdrop-blur-md border-4 border-rose-200 rounded-3xl p-6 md:p-8 shadow-2xl mb-12 relative">
        <div ref={scrollRef} className="h-80 md:h-96 overflow-y-auto pr-2 md:pr-4 custom-scrollbar text-center text-rose-600 space-y-3 font-medium text-lg sm:text-xl md:text-2xl">
          {ilyArray.map(num => (<div key={num} className="opacity-90">{num}. I love you</div>))}
        </div>
      ...
      <div className="bg-white p-8 md:p-10 rounded-3xl shadow-2xl border-2 border-rose-200 text-center w-full max-w-xl transform hover:-translate-y-2 transition-transform duration-300">
        ...
        <div className="relative">
          <input type="text" value={inputValue} onChange={handleInputChange} placeholder="Type it here..." className="w-full px-6 py-5 md:px-8 md:py-6 bg-pink-50 border-4 border-rose-300 rounded-2xl focus:outline-none focus:border-rose-500 focus:ring-4 focus:ring-rose-200 transition-all text-center text-2xl md:text-3xl text-rose-800 font-bold placeholder-pink-300 uppercase tracking-widest shadow-inner cursor-text" />
          {(inputValue.toLowerCase().trim() === 'ily' || inputValue.toLowerCase().includes('i love you') || inputValue.toLowerCase().includes('1000')) && (<Check className="absolute right-4 md:right-6 top-1/2 -translate-y-1/2 text-green-500 w-8 h-8 md:w-10 md:h-10 animate-bounce" />)}
        </div>
        <p className="text-base md:text-lg text-rose-500 mt-6 italic font-bold px-2">Hint: Type "ily", "I love you", or "1000"</p>
      </div>
    </div>
  );
}
